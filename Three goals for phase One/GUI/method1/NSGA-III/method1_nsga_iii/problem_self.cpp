...
void CProblemSelf::loadEnvironment(int num_a, int num_t)
{
	...
	fstream f("agents.txt", ios::in);
	for (int i = 0; i < num_a; i++)
	{
		f >> agt[i].x >> agt[i].y >> agt[i].res >> agt[i].tskn;
	...
	{
		f >> tsk[i].x >> tsk[i].y >> tsk[i].start_t >> tsk[i].end_t >> tsk[i].res_cost >> tsk[i].time_cost >> tsk[i].gain >> tsk[i].agtn;
...
bool CProblemSelf::Evaluate(CIndividual* indv) const
{
	...
	for (int i = 0; i < num_agent; i++)
	{
		...
		for (int j = n; j < n + agt[i].tskn; j++)
		{
			if (flag[x[j]] == 0)
			{
				double travel_time = sqrtf((tsk[x[j]].x - cur_x) * (tsk[x[j]].x - cur_x) + (tsk[x[j]].y - cur_y) * (tsk[x[j]].y - cur_y));				double travel_cost = travel_time;
				double cost_time = 0;
				double begin_time = 0;
				...
				{
					f[0] = f[0] - 1.0 / num_task; 					f[1] = f[1] - tsk[x[j]].gain / max_gain; 					f[2] = f[2] + (travel_cost + tsk[x[j]].res_cost) / (500 * num_agent); 					if (arrival_time > tsk[x[j]].start_t) {
		...
		n = n + agt[i].tskn; 		if (tsum > f[3]) 			f[3] = tsum;
	}
	f[3] = f[3] / 160; 	return true;
}
