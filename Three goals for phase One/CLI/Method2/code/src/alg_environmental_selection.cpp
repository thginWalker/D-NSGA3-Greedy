...
vector<double> TranslateObjectives(CPopulation* ppop, const CNondominatedSort::TFronts& fronts)
{
	...
	for (size_t f = 0; f < NumObj; f += 1)
	{
		...
		for (size_t i = 0; i < fronts[0].size(); i += 1) // min values must appear in the first front 最小值必须出现在第一个前面
		{
...
void EnvironmentalSelection(CPopulation* pnext, CPopulation* pcur, vector<CReferencePoint> rps, size_t PopSize)
{
	...
	fronts.erase(fronts.begin() + last, fronts.end()); // remove useless individuals 移除无用的个体
...
// ----------------------------------------------------------------------
////////////////////////////////////////////////////////////////
void EnvironmentalExchange(CPopulation* pnext, CPopulation* pcur, CPopulation* migrate, std::vector<CReferencePoint> rps, std::size_t PopSize)
{
	//CPopulation& cur = *pcur, & next = *pnext;个人更改
	//next.clear();
	CPopulation& cur = *pcur, & next = *pnext;
	//next.clear();
	for (int i = 0; i < (*migrate).size(); i++)
	{
		next.push_back((*migrate)[i]);
	}
	//////////////////////将解集添加到种群中


	// ---------- Step 4 in Algorithm 1: non-dominated sorting ---------- 第4步:非支配排序
	CNondominatedSort::TFronts fronts = NondominatedSort(cur);



	// ---------- Steps 5-7 in Algorithm 1 ---------- 添加非支配层的数目到数量大于等于N
	vector<size_t> considered; // St
	size_t last = 0, next_size = 0;
	while (next_size < PopSize)
	{
	...
	// ---------- Steps 9-10 in Algorithm 1 ----------  如果选择的层数正好等于种群个数N,则不需要进行下面的操作
	if (next.size() == PopSize) return;


	// ---------- Step 14 / Algorithm 2 ---------- 规范化目标并创建参考点集合
	vector<double> ideal_point = TranslateObjectives(&cur, fronts);//理想点

	vector<size_t> extreme_points;
	FindExtremePoints(&extreme_points, cur, fronts);//找到极值点

	vector<double> intercepts;
	ConstructHyperplane(&intercepts, cur, extreme_points);//构造超平面

	NormalizeObjectives(&cur, fronts, intercepts, ideal_point);//规范化目标


	// ---------- Step 15 / Algorithm 3, Step 16 ----------  St成员分配给相对应的参考点(关联操作)
	Associate(&rps, cur, fronts);

	// ---------- Step 17 / Algorithm 4 ---------- (小生境保存操作)
	while (next.size() < PopSize)
	{
		size_t min_rp = FindNicheReferencePoint(rps);

		int chosen = SelectClusterMember(rps[min_rp]);//选择聚合成员
		if (chosen < 0) // 若某参考点无聚合个体，则删除该个体
		{
			rps.erase(rps.begin() + min_rp);
		}
		else
...
///保存迁移学习解集saveLearningSolution
void saveLearningSolution(CPopulation* pnext, CPopulation* pcur, vector<CReferencePoint> rps, size_t PopSize, int islandnum)
{
	CPopulation& cur = *pcur, & next = *pnext;
	next.clear();
