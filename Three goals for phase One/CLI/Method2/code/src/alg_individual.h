...
public:
	...
	static void SetTargetProblem(const BProblem& p) { target_problem_ = &p; }		static void loadTargetProblem(BProblem& p) { target_problem_ = &p; }
	static void removedup(CIndividual* ind, int agtnum);
	static const BProblem& TargetProblem();
private:
	TDecVec variables_;		TObjVec objectives_;				TObjVec actual_objectives_;				TObjVec converted_objectives_;
