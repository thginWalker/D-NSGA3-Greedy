...
int main()
{
	...
	size_t  migratenum = nsgaiii.getMigrateNum();//用配置文件读取
	...
	for (unsigned int i = 0; i < solutions.size(); i++)//整个条数
	{
		cout << solutions[i] << endl;
	}


	string logfname = "Results\\" + nsgaiii.name() + "-" + problem[0]->name() + "-Run.txt"; // e.g. NSGAIII-DTLZ1(3)-Run0.txt
	ofstream fout;
	fout.open(logfname);
	...
	for (int k = 0; k < islandnum; k++)
	{
		string logfname = "Results\\" + nsgaiii.name() + "-" + problem[k]->name() + "-Run" + to_string(k) + ".txt"; // e.g. NSGAIII-DTLZ1(3)-Run0.txt
		ofstream fout;
		fout.open(logfname);
